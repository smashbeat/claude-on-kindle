...
- **Optional: require a proxy-injected secret header.** For belt-and-suspenders against the co-located-process case, set `KT_AUTH_HEADER` + `KT_AUTH_SECRET`; the app then 403s any request missing that header. Have your trusted proxy inject it — with Cloudflare: **Rules → Transform Rules → Modify Request Header → Set static**, header `X-KT-Auth` = your secret, scoped to your hostname. The header is added at the **edge**, so the browser never sees the secret and it stays ancient-browser-compatible. A process that reaches the port directly (without going through the proxy) won't have the header and is rejected.
